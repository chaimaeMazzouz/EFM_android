...
public class Modifier_Produit extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        typeMouvementChx=findViewById(R.id.TypeOp_M);
    }
    ...
    public void rechercherProduit(View view) {
        ...
        desinationEdt.setText(p.getDescription());
        prixUnitaireEdt.setText(p.getMontant().toString());
        if(p.getTypeOperation()){
    ...
    public void modifierProduit(View view) {
        if(TextUtils.isEmpty(desinationEdt.getText().toString())){
            showToast("Il faut remplir la description");
            return;
        }
        if(TextUtils.isEmpty(prixUnitaireEdt.getText().toString())){
            showToast("Il faut remplir le montant");
            return;
        }
        if(code==-1){
        ...
        MainActivity.db.produitDAO().modifierProduit(new Produit(code,designation,prixUnitaire,typeMouvement));
